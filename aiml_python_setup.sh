...
#               Jupyter, NumPy, Pandas, TensorFlow, PyTorch
...
#  3. Python 3 + pip + venv
# --------------------------------------------------------------------------- #
print_step "3/10 — Installing Python 3, pip, venv..."
apt-get install -y python3 python3-pip python3-venv python3-dev || print_error "Python install failed"
update-alternatives --install /usr/bin/python python /usr/bin/python3 1 2>/dev/null || true
python3 --version && print_success "Python installed: $(python3 --version)" || print_error "Python not found"
pip3 --version && print_success "pip installed: $(pip3 --version)" || print_error "pip not found"
 
# --------------------------------------------------------------------------- #
#  4. Django + AI/ML Packages
# --------------------------------------------------------------------------- #
print_step "4/10 — Installing Django and AI/ML packages..."
pip3 install --break-system-packages django || print_error "Django install failed"
pip3 install --break-system-packages djangorestframework || true
pip3 install --break-system-packages django-cors-headers || true
pip3 install --break-system-packages psycopg2-binary || true
pip3 install --break-system-packages python-dotenv || true
pip3 install --break-system-packages pillow || true
pip3 install --break-system-packages requests || true
pip3 install --break-system-packages numpy || true
pip3 install --break-system-packages pandas || true
pip3 install --break-system-packages matplotlib || true
pip3 install --break-system-packages scikit-learn || true
pip3 install --break-system-packages jupyter notebook ipykernel || true
pip3 install --break-system-packages tensorflow || true
pip3 install --break-system-packages torch torchvision || true
pip3 install --break-system-packages openai langchain fastapi uvicorn || true
django-admin --version && print_success "Django installed: $(django-admin --version)" || print_error "Django not found"
print_success "AI/ML packages installation attempted."
 
# --------------------------------------------------------------------------- #
#  5. PostgreSQL 16 + pgAdmin 4
# --------------------------------------------------------------------------- #
print_step "5/10 — Installing PostgreSQL 16..."
...
#  6. Docker
# --------------------------------------------------------------------------- #
print_step "6/10 — Installing Docker..."
...
# --------------------------------------------------------------------------- #
print_step "7/10 — Installing Visual Studio Code..."
...
#  8. Google Chrome
# --------------------------------------------------------------------------- #
print_step "8/10 — Installing Google Chrome..."
...
#  9. Microsoft Teams
# --------------------------------------------------------------------------- #
print_step "9/10 — Installing Microsoft Teams..."
...
#  10. Postman
# --------------------------------------------------------------------------- #
print_step "10/10 — Installing Postman..."
...
# --------------------------------------------------------------------------- #
echo -e "\n${CYAN}============================================================${NC}"
echo -e "${GREEN}   ✔  AIML SETUP COMPLETE!${NC}"
echo -e "${CYAN}============================================================${NC}"
echo ""
echo -e "${YELLOW}Installed Software Summary:${NC}"
echo -e "  ${GREEN}✔${NC} Git          : $(git --version 2>/dev/null || echo 'Check manually')"
echo -e "  ${GREEN}✔${NC} Python       : $(python3 --version 2>/dev/null || echo 'Check manually')"
echo -e "  ${GREEN}✔${NC} pip          : $(pip3 --version 2>/dev/null | awk '{print $1,$2}' || echo 'Check manually')"
echo -e "  ${GREEN}✔${NC} Django       : $(django-admin --version 2>/dev/null || echo 'Check manually')"
...
echo -e "  • PostgreSQL    : sudo systemctl status postgresql"
echo -e "  • Docker        : sudo systemctl status docker"
echo -e "  • Jupyter       : jupyter notebook"
echo -e "  • Django project: django-admin startproject myproject"
